async function fetchComplianceData() {
  ...
  const isGdprSelected = document.getElementById("gdprRadio").checked;
  const isPopaiSelected = document.getElementById("popaiRadio").checked;

  // Validate that at least one option is selected
  if (!isGdprSelected && !isPopaiSelected) {
    alert("Please select at least one compliance option.");
    return;
  }

  // Set API endpoint based on selection
  const apiUrl = isGdprSelected
    ? "https://render-test-844r.onrender.com/check_compliance"
    : "https://render-test-2-chxb.onrender.com/check_compliance";
...
function populateArticles(nonCompliantArticles, compliantArticles) {
  ...
  nonCompliantArticles.forEach((article) => {
    ...
    const complianceStatus =
      reasonParagraphs[0] || "Compliance status: Non-compliant";
  ...
  compliantArticles.forEach((article) => {
    ...
    requirementText.textContent =
      article.Requirement || "No requirements provided.";
